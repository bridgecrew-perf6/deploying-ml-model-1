...
def run_sanity_check(test_dir):
    ...
    test_function_names = list(filter(
        lambda x: inspect.isfunction(getattr(module, x)) and not x.startswith(
            '__'), dir(module)))
    ...
    if not test_functions_for_get:
        ...
        if not TEST_FOR_GET_METHOD_RESPONSE_CODE:
            ...
            WARNING_COUNT += 1
            print(
                FAIL_COLOR + "Your test case for GET() does not seem to be testing the response code.\n")

        if not TEST_FOR_GET_METHOD_RESPONSE_BODY:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(
                FAIL_COLOR + "Your test case for GET() does not seem to be testing the CONTENTS of the response.\n")
    ...
    if not test_functions_for_post:
        ...
        if len(test_functions_for_post) == 1:
            ...
            WARNING_COUNT += 1
            print(
                FAIL_COLOR + "Only one test case was detected for the POST() method.")
            print(
                FAIL_COLOR + "Please make sure you have two test cases for the POST() method." +
                "\nOne test case for EACH of the possible inferences (results/outputs) of the ML model.\n")
            SANITY_TEST_PASSING = False
    ...
    print(
        WARN_COLOR + "This is a heuristic based sanity testing and cannot guarantee the correctness of your code.")
    print(
        WARN_COLOR + "You should still check your work against the rubric to ensure you meet the criteria.")
